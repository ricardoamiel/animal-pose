...
def is_valid_visibility(v):
    """Valida que el valor de visibilidad sea 0, 1, o 2"""
    return v in [0, 1, 2]
    ...
    try:
        ...
        for raw_line in lines_raw:
            for instance in raw_line.strip().split(";"):
                ...
                # Validar coordenadas del bbox
                if not all(is_valid_coordinate(x, y) for x, y in [(x_center, y_center), (w_norm, h_norm)]):
                    print(f"Warning: Invalid bbox coordinates in {label_path}")
                    continue
                ...
                for i in range(0, len(keypoints_raw), 3):
                    ...
                    if v == 0.0:
                        ...
                        v_int = int(2)  # Valor de visibilidad para puntos visibles
                        
                        # Validar coordenadas y visibilidad de keypoints
                        if not is_valid_coordinate(x_norm, y_norm) or not is_valid_visibility(v_int):
                            print(f"Warning: Invalid keypoint coordinates or visibility in {label_path}")
                            continue
                            
                        keypoints.extend([x_norm, y_norm, v_int])

                line = [str(class_id), f"{x_center:.6f}", f"{y_center:.6f}", 
                       f"{w_norm:.6f}", f"{h_norm:.6f}"] + [str(kp) for kp in keypoints]
...
for subset in SUBSETS:
    ...
    output_label_dir = os.path.join(DATA2_DIR, subset, "labels")
